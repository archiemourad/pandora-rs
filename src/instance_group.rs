...
impl InstanceGroup {
    ...
    pub fn new(device: &wgpu::Device, transforms: &[Transform]) -> Self {
        ...
        Self {
            buffer,
            count: gpu_transforms.len() as u32,
        }
    }

    pub fn update(&self, queue: &wgpu::Queue, transforms: &[Transform]) {
        let gpu_transforms: Vec<GPUTransform> = transforms.iter().map(Transform::to_gpu).collect();
        queue.write_buffer(&self.buffer, 0, bytemuck::cast_slice(&gpu_transforms));
    }

    pub fn draw(&self, render_pass: &mut wgpu::RenderPass, model: &Model) {
        ...
        if let Some(index_buffer) = model.mesh.index_buffer() {
            ...
            render_pass.draw_indexed(0..model.mesh.index_count(), 0, 0..self.count);
        } else {
            render_pass.draw(0..model.mesh.vertex_count(), 0..self.count);
        }
    }
}
